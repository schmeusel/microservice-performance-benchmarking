import {
    ...
    experimentResultReducer, manualDecisionReducer,
...
describe('Test experimentReducers', () => {
    describe('manualDecisionReducer', () => {
        it('should return the initial state', () => {
            const result = manualDecisionReducer(undefined, {});
            expect(result)
                .toEqual(false);
        });

        it('should handle MANUAL_DECISION', () => {
            const actionStub = {
                type: ActionTypes.EXPERIMENT.MANUAL_DECISION,
                data: true,
            };
            const result = manualDecisionReducer(false, actionStub);
            expect(result)
                .toEqual(actionStub.data);
        });
    });
    ...
    describe('experimentResultReducer', () => {
        ...
        it('should handle EXPERIMENT.DECISION.RESULT', () => {
            ...
            expect(resultWithSucceed)
                .toEqual({
                    async: asyncStub,
                    value: valueStubSucceed,
                });
            expect(resultWithFail)
                .toEqual({
                    async: asyncStub,
                    value: valueStubFail,
                });
