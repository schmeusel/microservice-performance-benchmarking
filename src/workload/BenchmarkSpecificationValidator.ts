...
class BenchmarkSpecificationValidator implements Validator {
	...
	public validate(benchmarkSpecification: BenchmarkSpecification): boolean {
		return this.validateConfiguration(benchmarkSpecification.configuration)
	...
	private validateSLACondition(condition: SLACondition): boolean {
		...
		Object.keys(condition).forEach(key => {
			if (!allowedKeys.includes(key)) {
				throw new ValidationError(`Only "${allowedKeys.join(', ')}" are allowed to specify an SLA condition, not "${key}"`);
			}
		})
